...
//get starting phase from backup
$result = mysql_query("SELECT dorder FROM battle WHERE id = $game_id");
$prevfaza = mysql_fetch_assoc($result);

if ($prevfaza['dorder'] == '0') {//Westeros Card 1 phase
    mysql_query("UPDATE `game` SET faza='0' WHERE `id` = $game_id");

    $result = mysql_query("SELECT turn, terr FROM game WHERE id = $game_id");
    ...
    if ($WC["WC1_".$turn['turn']] == '0') {//Last days of Summer
        ...
        $result = mysql_query("SELECT control, supply FROM ".$turn['terr']);
