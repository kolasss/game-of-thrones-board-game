...
//plus VBS power
$result = mysql_query("SELECT attacker, defender FROM battle WHERE id=$game_id");
$side = mysql_fetch_assoc($result);
$result = mysql_query("SELECT fiefdom1 FROM game where id=$game_id");
$fiefdom = mysql_fetch_assoc($result);

if ($fiefdom['fiefdom1'] == $side['attacker']) {
	mysql_query("UPDATE `battle` SET currentplayer = 0, aCS=aCS+1 WHERE `id` = $game_id");
} else if ($fiefdom['fiefdom1'] == $side['defender']) {
	mysql_query("UPDATE `battle` SET currentplayer = 0, dCS=dCS+1 WHERE `id` = $game_id");
}

//Combat resolution phase
mysql_query("UPDATE `game` SET faza = 11, Blade = 1 WHERE `id` = $game_id");
